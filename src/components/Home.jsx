...
function Home() {
  ...
  let filterData = data.matchDetails.filter((singleData) =>
    singleData["matchDetailsMap"]
  )
        ...
        matchesData.map((data, index) => (
          data.matchDetailsMap.match.map((singleMatch) => (
            <Link to={`/matchDetail/${singleMatch.matchInfo.matchId}`}>
             <h1>{singleMatch.matchInfo.team1.teamSName} vs {singleMatch.matchInfo.team2.teamSName} in {data.matchDetailsMap.key}{singleMatch.matchInfo.status}</h1>
            </Link>
